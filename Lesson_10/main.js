...
//
let formUserInfo = document.createElement('form');
formUserInfo.setAttribute('action', '');
formUserInfo.setAttribute('id', 'formUserInfo');

let inputUserName = document.createElement('input');
inputUserName.setAttribute('type', 'text');
inputUserName.setAttribute('name', 'userName');
inputUserName.setAttribute('id', 'userNameId');
inputUserName.setAttribute('placeholder', 'Enter your name');
let br = document.createElement('br');

let inputUserSurname = document.createElement('input');
inputUserSurname.setAttribute('type', 'text');
inputUserSurname.setAttribute('name', 'userSurname');
inputUserSurname.setAttribute('id', 'userSurnameId');
inputUserSurname.setAttribute('placeholder', 'Enter your surname')
let clone_br_first = br.cloneNode(true);

let inputUserAge = document.createElement('input');
inputUserAge.setAttribute('type', 'number');
inputUserAge.setAttribute('name', 'userAge');
inputUserAge.setAttribute('id', 'userAgeId');
inputUserAge.setAttribute('placeholder', 'Enter your age');
let clone_br_second = br.cloneNode(true);

let buttonSendInfo = document.createElement('button');
buttonSendInfo.setAttribute('type', 'submit');
buttonSendInfo.setAttribute('id', 'buttonSendUserInfo');
buttonSendInfo.innerText = 'Send info';

document.body.appendChild(formUserInfo);
formUserInfo.append(inputUserName, br, inputUserSurname, clone_br_first, inputUserAge, clone_br_second, buttonSendInfo);
...
buttonSendInfo.onclick = function () {
    ...
    console.log(objUser);
    document.write(`Name: ${objUser.name}  <br>Surname: ${objUser.surname}<br> Age: ${objUser.age}<br>`);
    // ------------------------------------без створення об'єкту objUser----------------------------------------
    // document.write(`Name: ${inputUserName.value} <br>Last Name: ${inputUserSurname.value}<br> Age: ${inputUserAge.value}`)
}
// -------------------------------------доступ через id---------------------------------------

// let buttonSendInfo = document.forms[0].buttonSendUserInfo;
// buttonSendInfo.onclick = function () {
//     let name = document.getElementById("userNameId").value;
//     let surname = document.getElementsByName("userSurname")[0].value;
//     let age = document.getElementById("userAgeId").value;
...
// }
// ==========================
// є сторінка, на якій є блок, якому знаходиться цифра. написати код, який при кожному перезавантажені
// сторінки буде додавати до неї +1
let counter = localStorage.getItem('countOfSiteLoading') || 0;
let plusOne = +counter + 1;
localStorage.setItem('countOfSiteLoading', JSON.stringify(plusOne));
let textBlock = document.getElementById('textNumber');
// let textBlock = document.querySelector('#textNumber');//другий варіант пошуку елементу DOM
let sum = plusOne + +textBlock.innerText;
textBlock.innerText = `${sum}`;
// ==========================
...
let time = new Date().toTimeString().slice(0);
let obj = {date, time};
if (!localStorage.sessionsInfo) {
    info.push(obj);
    localStorage.sessionsInfo = JSON.stringify(info)
} // localStorage.setItem('sessionsInfo', JSON.stringify(info))
else {
    info = JSON.parse(localStorage.sessionsInfo);
    info.push(obj);
    localStorage.sessionsInfo = JSON.stringify(info)
}
...
// let infoAboutSessions = localStorage.getItem('sessionsInfo');
// let info = infoAboutSessions ? JSON.parse(infoAboutSessions) : [];
// info.push(obj);
// localStorage.sessionsInfo = JSON.stringify(info); //localStorage.setItem('sessionsInfo', JSON.stringify(info));
...
for (let infoElement of info) {
    ...
    li.innerText = `Дата: ${infoElement.date}, час: ${infoElement.time}`;
    ol.appendChild(li);
}
// =========================
...
let jokesArr = jokesArrFul.slice(0, 100);
let startIndex = 0;
let endIndex = 10;
...
function getJokes() {
    let jokes = jokesArr.slice(startIndex, endIndex);
    for (let joke of jokes) {
        let cardWithJoke = document.createElement('div');
        cardWithJoke.classList.add('cardJokes');
        let question = document.createElement('p');
        question.classList.add('question');
        question.innerText = joke.setup;
        let answer = document.createElement('p');
        answer.classList.add('answer');
        answer.innerText = joke.punchline;
        wrapper.appendChild(cardWithJoke);
        cardWithJoke.append(question, answer);
    }
}
...
divForButtons.append(button_1, button_2)
button_1.innerText = 'prev';
button_2.innerText = 'next';
...
function check() {
    startIndex > 0 ? button_1.disabled = false : button_1.disabled = true;
    endIndex >= jokesArr.length ? button_2.disabled = true : button_2.disabled = false;
}
...
textDiv.id = 'text';
textDiv.innerText = 'Дуже цікавий та корисний текст :)';
let buttonTextTheft = document.createElement('button');
let buttonTextAppearance = document.createElement('button');
buttonTextTheft.innerText = 'Текст, зникни!';
buttonTextAppearance.innerText = "Текст, з'явися!";
buttonTextAppearance.style.display = 'none';
document.body.append(textDiv, buttonTextTheft, buttonTextAppearance)
buttonTextTheft.addEventListener('click', function () {
    textDiv.style.display = 'none';
    buttonTextAppearance.style.display = 'flex';
    buttonTextTheft.style.display = 'none';
})
buttonTextAppearance.addEventListener('click', function () {
    textDiv.style.display = 'flex';
    buttonTextAppearance.style.display = 'none';
    buttonTextTheft.style.display = 'flex';
})
// ---------------------------------or-----------------------------------
let flag;
...
textBlock2.id = 'text';
textBlock2.innerText = 'Ще один корисний текст для навчання :)';
let buttonForFlag = document.createElement('button');
buttonForFlag.innerText = 'Прибрати текст!';
section.append(textBlock2, buttonForFlag);
buttonForFlag.addEventListener('click',function () {
    if (flag === true) {
        buttonForFlag.innerText = 'Прибрати текст!';
        flag=false;
        textBlock2.style.display = 'flex';
    } else {
        buttonForFlag.innerText = 'Відкрити текст!';
...
//     кнопку зчитати інформацію з інпуту та перевірити вік чи менше він ніж 18,
//     та повідомити про це користувача
let formAge = document.createElement('form');
document.body.appendChild(formAge);
let inputAge = document.createElement('input');
formAge.appendChild(inputAge);
inputAge.type = 'number';
inputAge.placeholder = 'Enter your age';
inputAge.name = 'userName';
let buttonSubmitAge = document.createElement('button');
formAge.appendChild(buttonSubmitAge);
buttonSubmitAge.innerText = 'Send age';
let age;
formAge.addEventListener('submit', function (e) {
    e.preventDefault();
    let infoFromInput = new FormData(formAge);
    age = infoFromInput.get('userName');
    if (age < 18) {
        // можна скористатися alert('Ваш вік менший за 18 років')
        let textForUser = document.createElement('p');
        document.body.insertBefore(textForUser, input);
        textForUser.innerText = 'Ваш вік менший за 18 років';

    } else if (age >= 18) {
        let textForUser = document.createElement('p');
        document.body.insertBefore(textForUser, input);
        textForUser.innerText = 'Ви повнолітня особа';
...
button.innerText = 'Відправити дані про вік';
let ageFromInput;
button.addEventListener('click', function () {
    ageFromInput = input.value;
    console.log(ageFromInput);
    if (ageFromInput < 18) {
        let text = document.createElement('p');
        document.body.insertBefore(text, divForInputs);
        // document.body.appendChild(text);
        text.innerText = 'Малим тут не місце!';
    }
})
...
document.body.appendChild(divForInputs);


let rowQuantity = document.createElement('input');
rowQuantity.name = 'rowQuantity';
rowQuantity.type = 'number';
rowQuantity.placeholder = 'Quantity of rows';


let numberOfCells = document.createElement('input');
numberOfCells.name = 'numberOfCells';
numberOfCells.type = 'number';
numberOfCells.placeholder = 'Number of cells';


let cellContent = document.createElement('input');
cellContent.name = 'cellContent';
cellContent.type = 'text';
cellContent.placeholder = 'Cell content';


let buttonFormTable = document.createElement('button');
buttonFormTable.type = 'submit';
buttonFormTable.innerText = 'Form table!';

divForInputs.append(rowQuantity, numberOfCells, cellContent, buttonFormTable);

function formTableWithContent() {
    let table = document.createElement('table');
    document.body.insertBefore(table, h1);
    rowQuantity.style.display = 'none';
    numberOfCells.style.display = 'none';
    cellContent.style.display = 'none';
    buttonFormTable.style.display = 'none';
    for (let i = 0; i < rowQuantity.value; i++) {
        let row = document.createElement('tr');
        table.appendChild(row);
        for (let j = 0; j < numberOfCells.value; j++) {
            let cell = document.createElement('td');
            row.appendChild(cell);
            cell.innerText = cellContent.value;
        }
    }
}

buttonFormTable.addEventListener('click', formTableWithContent);
//


//   // (Додаткова частина для завдання)
//   <h1>Усього: <span id="cash">100</span><span> грн</span></h1>
let h1 = document.createElement('h1');
h1.innerText = 'Усього: ';
let spanCash = document.createElement('span');
spanCash.id = 'cash';
spanCash.innerText = '100';
let span = document.createElement('span');
span.innerText = ' грн';
document.body.appendChild(h1);
h1.append(spanCash, span);
...
let infoOfTime = localStorage.getItem('eachLoadingTimeInfo');
let loadingTime = infoOfTime ? JSON.parse(infoOfTime) : [];
loadingTime.push(Date.now())
localStorage.setItem('eachLoadingTimeInfo', JSON.stringify(loadingTime));


if ((+loadingTime[loadingTime.length - 1] - +loadingTime[loadingTime.length - 2]) >= 10000) {
    let sum = localStorage.getItem('price');
    sum = +sum + 10;
    localStorage.setItem('price', String(sum));
    spanCash.innerText = String(+spanCash.innerText + sum);
} else {
    let number = +localStorage.getItem('price') + +spanCash.innerText;
    spanCash.innerText = String(number);
}